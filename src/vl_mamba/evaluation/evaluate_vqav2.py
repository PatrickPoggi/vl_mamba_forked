...
class VQAv2EvalDataset(BaseEvalDataset):
    """VQA v2 evaluation dataset."""
...
class DataArguments(BaseDataArguments):
    ...
    eval_dataset_subset: str = field(default="vqa_v2")
    split: Literal["validation", "teststd", "testdev", "test"] = field(default="validation")
...
def evaluate() -> None:
    ...
    for idx in pbar:
        ...
        question_ids.append(instance.raw_target["question_id"])  # type: ignore[report]
        logger.info(f"Question: {instance.raw_target['question']}, Answer: {output_str}")  # type: ignore[report]
        if instance.raw_target.get("answers", None):  # type: ignore[report]
            groundtruths.append(instance.raw_target["answers"])  # type: ignore[report]
            score += vqa_v2_score(Counter(instance.raw_target["answers"]).get(output_str, 0))  # type: ignore[report]
            pbar.set_postfix({"Accuracy": f"{(score / (idx + 1)):.4f}"})
